...
public class Youtube1 {
    ...
    setAttrib setAttribY = new setAttrib("Youtube1","com.google.android.youtube");
    ...
    public boolean enterSearchText(){
        boolean value = false;
        try{
            UiSelector selector = new UiSelector().resourceId("com.google.android.youtube:id/search edit_text");
            UiObject object = device.findObject(selector);
...
//                UiSelector selector = new UiSelector().descriptionContains("Fishball | #AtinAngMundo - 2 minutes");
                UiSelector selector = new UiSelector().descriptionContains("Fishball | #AtinAngMundo - 2 minutes, 13 seconds - Go to channel - globeph - 1.7M views - 1 year ago - play video");
                UiObject object = device.findObject(selector);
...
//                UiSelector selectorSec = new UiSelector().resourceId("com.google.android.youtube:id/player_view");
                UiSelector selectorSec = new UiSelector().className("android.view.ViewGroup").index(3);
                UiObject objectSec = device.findObject(selectorSec);
                if (object.exists()) {
                    object.click();
                    CVT = new Timestamp(new Date().getTime());
                    Log.d(GlobalVariables.Tag_Name, "Clicked on Video successfully 1:" + CVT);
                    value = true;
                    break;
                }
                if (objectSec.exists()) {
                    objectSec.click();
                    CVT = new Timestamp(new Date().getTime());
                    Log.d(GlobalVariables.Tag_Name, "Clicked on Video successfully 2:" + CVT);
                    value = true;
                }
            }
        }catch (Exception e){
    ...
    public boolean videoDisplayed() {
        boolean value = false;
        try {
            UiSelector selector = new UiSelector().descriptionContains("Expand Mini Player");
            UiObject object = device.findObject(selector);
            if (object.waitForExists(GlobalVariables.OneMin_Timeout)) {
...
//            UiSelector selector = new UiSelector().resourceId("com.google.android.youtube:id/list_item_text_secondary");
            UiSelector selector = new UiSelector().className("android.view.ViewGroup").index(0).clickable(true).enabled(true);
            UiObject object = device.findObject(selector);
            object.click();

            UiSelector selectorq = new UiSelector().resourceId("com.google.android.youtube:id/bottom_sheet_title_resolution");
            UiObject objectq = device.findObject(selectorq);

            if (objectq.waitForExists(GlobalVariables.OneMin_Timeout)) {
                videoQuality = objectq.getText();
