...
public class GCash {
    ...
    public int testRun(UiDevice device) {
        ...
        try {
            ...
            runTest = navClose();
            if(!runTest){
                DataHolder.getInstance().setFailureReason("Unable to close nav logout");
                return 1;
            }
            runTest = logOut();
            if(!runTest){
    ...
    public boolean PINDisplayed() {
        boolean value = false;
        try{
//            UiSelector selector = new UiSelector().resourceId("com.globe.gcash.android:id/btn_number6").index(2);
            UiSelector selectorpin = new UiSelector().resourceId("com.globe.gcash.android:id/btn_mpin");
            UiObject objectpin = device.findObject(selectorpin);
            objectpin.click();

            UiSelector selector = new UiSelector().resourceId("com.globe.gcash.android:id/cl_num_pad");
        ...
        try {
            ...
            while (stopWatch.getTime() <= GlobalVariables.OneMin_Timeout) {
                for(int ab = 0;ab <= 3;ab++){
                    //first PIN
                    UiSelector firstSelector = new UiSelector().resourceId("com.globe.gcash.android:id/tv_six");
                    UiObject firstObject = device.findObject(firstSelector);

                    //second PIN
                    UiSelector secondSelector = new UiSelector().resourceId("com.globe.gcash.android:id/tv_two");
                    UiObject secondObject = device.findObject(secondSelector);

                    //third PIN
                    UiSelector thirdSelector = new UiSelector().resourceId("com.globe.gcash.android:id/tv_seven");
                    UiObject thirdObject = device.findObject(thirdSelector);

                    //fourth PIN
                    UiSelector fourthSelector = new UiSelector().resourceId("com.globe.gcash.android:id/tv_five");
    ...
    public boolean homePage() {
        ...
        try {
            ...
            while (stopWatch.getTime() <= GlobalVariables.OneMin_Timeout) {
                UiSelector selector = new UiSelector().resourceId("com.globe.gcash.android:id/iv_currency");
                UiObject object = device.findObject(selector);
        ...
        try{
            ...
            while (stopWatch.getTime() <= GlobalVariables.OneMin_Timeout) {
                ...
                if (removeAutoObject.exists())
                {
                    ...
                    contactObject.setText("09212893937");
                    Thread.sleep(2000);
    ...
    public boolean successfullySent(){
        ...
        try{
            ...
            UiSelector selector = new UiSelector().textContains("Successfully Sent");
            UiObject object = device.findObject(selector);
    ...
    public boolean navClose(){
        ...
        try{
            ...
            UiSelector selectorOK = new UiSelector().textContains("Ok");
            ...
            if(object.waitForExists(GlobalVariables.OneMin_Timeout)){
                ...
                Log.d(GlobalVariables.Tag_Name,"Close successfully: " + unUsableTime);
                value = true;
            }

        }catch (Exception e){
            e.printStackTrace();
            Log.d(GlobalVariables.Tag_Name,"Error Clicking Close Button");
        }
        return value;
    }
    ...
    public boolean logOut(){
        ...
        try{
            ...
            UiSelector selector = new UiSelector().descriptionContains("Profile");
            UiObject object = device.findObject(selector);


            UiSelector selectorOk = new UiSelector().textContains("Log Out");
            UiObject objectOk = device.findObject(selectorOk);
